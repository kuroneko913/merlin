...
from models.dnn_keras import DNN, RNN, HWAY,HWAY_Multi
...
#    pretrain_set_x = train_set_x
    ...
    if model_type == 'DNN':
        ...
        dnn_model = HWAY_Multi(n_in= n_ins, hidden_layer_size = hidden_layer_size, n_out = n_outs, 
                                         L1_reg = l1_reg, L2_reg = l2_reg, hidden_layer_type = hidden_layer_type, dropout_rate = dropout_rate)
    ...
    # use atr503-weights
    try:
        logger.info('load_weights')
        dnn_model.model.load_weights(os.path.dirname(nnets_file_name)+'/model_weights_atr503.h5')
        # Weights freez!
        for layer in dnn_model.model.layers[:-10]:
            layer.trainable=False
        # dnn_model.model.compile(loss='mse', optimizer=dnn_model.optimizer,metrics=['accuracy'])
        dnn_model.model.compile(loss='mse',optimizer=optimizers.SGD(lr=finetune_lr,momentum=momentum),metrics=['accuracy'])
        dnn_model.model.summary()
    except IOError:
        logger.debug("Not Found model_weights file.")
        dnn_model.model.summary()
        pass
...
#    finetune_lr = 0.000125
    ...
    while (epoch < training_epochs):
        ...
        while (not train_data_reader.is_finish()):
            ...
            history=dnn_model.model.fit(temp_train_set_x, temp_train_set_y, batch_size=batch_size, nb_epoch=1)
        train_data_reader.reset()
    ...
    # save_model_weights
    nnets_weights_file_name=os.path.dirname(nnets_file_name)+'/model_weights.h5'
    dnn_model.model.save_weights(nnets_weights_file_name)
    
    del dnn_model, best_dnn_model
...
def dnn_generation(valid_file_list, nnets_file_name, n_ins, n_outs, out_file_list):
    ...
    dnn_model = load_model(nnets_file_name)
    ...
    for i in xrange(file_number):  #file_number
        ...
        # for i,o in enumerate(zip(test_set_x,predicted_parameter)):
        #     print i,'test_set_x:\n',o[0]
        #     print 'predicted_parameter:\n',o[1]
        ...
        # # save_model_weights
        # nnets_weights_file_name=os.path.dirname(nnets_file_name)+'/model_weights.h5'
        # dnn_model.save_weights(nnets_weights_file_name)
        plot(dnn_model, to_file=os.path.dirname(nnets_file_name)+"/model.png", show_shapes=True)
...
def main_function(cfg):    
   ...
   # pdb.set_trace()
    ...
    nnets_file_name = '%s/%s_%s_%d_%s_%d.%d.train.%d.%f.rnn.model.h5' \
